...
public class HeartThermicAnomaly  extends BlockTileEntity<TileEntityHeartThermicAnomaly> {

    public HeartThermicAnomaly(String name, Material material, float hardness, float resistanse, SoundType soundType) {
    ...
    public void onBlockPlacedBy(World world, BlockPos pos, IBlockState state, EntityLivingBase placer,
    		ItemStack stack) {
    	if (!world.isRemote) {
    		TileEntityHeartThermicAnomaly tileEntity = getTileEntity(world, pos);
        	
    	}
    }

    @Override
    public Class<TileEntityHeartThermicAnomaly> getTileEntityClass() {

        return TileEntityHeartThermicAnomaly.class;
    }

    @Override
    public TileEntityHeartThermicAnomaly createTileEntity(World world, IBlockState blockState) {

        return new TileEntityHeartThermicAnomaly();
    }
    	...
    	if (!world.isRemote) {
            ...
            if((tileEntity.getLastLootTime()+STALKERMain.fillingTime*20<world.getTotalWorldTime())) {
                ...
                for (int i = 0; i < artefactCount; i++) {
                	...
                	int y= pos.getY();
                	int z = pos.getZ();
                	InventoryHelper.spawnItemStack(world, x,y, z, OtherMethods.returnLoot(STALKERMain.itemsThermicAnomaly)[rand.nextInt(OtherMethods.returnLoot(STALKERMain.itemsThermicAnomaly).length)]);
				}
