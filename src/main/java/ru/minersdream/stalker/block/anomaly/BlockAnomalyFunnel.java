...
public class BlockAnomalyFunnel extends Block implements IHasModel {
	...
	public BlockAnomalyFunnel(String name, int lighting, int damage, int timeregen, int stagespeed, int stagespeedfulldischarged) {
	...
	public void onEntityCollidedWithBlock(World worldIn, BlockPos pos, IBlockState state, Entity entityIn) {
		...
		tighteningEffect(worldIn, entityIn, pos, state.getValue(this.ANOMALYSTAGE));
		...
		else if (state.getValue(this.ANOMALYSTAGE) == 4) {
			if (!(entityIn instanceof EntityLivingBase)) {
				entityIn.setDead();
	...
	private void tighteningEffect(World worldIn, Entity entityIn, BlockPos pos, int stage) {
		double range = 1.0;
        ...
        for (Object entityObject : entities) {
            if (entityObject instanceof Entity) {
                ...
                if (entity instanceof Entity) {
                	double EntityPosX=entity.posX-0.5;
                	double EntityPosY=entity.posY-0.5;
                	double EntityPosZ=entity.posZ-0.5;
                	double moveX=0;
                	double moveY=0;
                	double moveZ=0;
