...
public class HeartGravitationalAnomaly extends BlockTileEntity<TileEntityHeartGravitationalAnomaly> {

    public HeartGravitationalAnomaly(String name, Material material, float hardness, float resistanse, SoundType soundType) {
    ...
    public void onBlockPlacedBy(World world, BlockPos pos, IBlockState state, EntityLivingBase placer,
    		ItemStack stack) {
    	if (!world.isRemote) {
    		TileEntityHeartGravitationalAnomaly tileEntity = getTileEntity(world, pos);
        	
    	}
    }

    @Override
    public Class<TileEntityHeartGravitationalAnomaly> getTileEntityClass() {

        return TileEntityHeartGravitationalAnomaly.class;
    }

    @Override
    public TileEntityHeartGravitationalAnomaly createTileEntity(World world, IBlockState blockState) {

        return new TileEntityHeartGravitationalAnomaly();
    }
        ...
        if (!world.isRemote) {
        	...
        	if((tileEntity.getLastLootTime()+STALKERMain.fillingTime*20<world.getTotalWorldTime())) {
                ...
                for (int i = 0; i <= artefactCount; i++) {
                	...
                	int y= pos.getY();
                	int z = pos.getZ();             
                	InventoryHelper.spawnItemStack(world, x,y, z, OtherMethods.returnLoot(STALKERMain.itemsGravitationalAnomaly)[rand.nextInt(OtherMethods.returnLoot(STALKERMain.itemsGravitationalAnomaly).length)]);
				}
