...
public class BlockAnomalySteam extends Block implements IHasModel {
	...
	public BlockAnomalySteam(String name, int lighting, float hardness, float resistanse, SoundType soundType, int damage,
		...
		setDefaultState(blockState.getBaseState().withProperty((IProperty) BlockAnomalySteam.ANOMALYSTAGE, 0));

	}

	@Override
	public void randomDisplayTick(IBlockState stateIn, World worldIn, BlockPos pos, Random rand) {
		if (stateIn.getValue(BlockAnomalySteam.ANOMALYSTAGE) == 0)
	...
	public void onEntityCollidedWithBlock(World worldIn, BlockPos pos, IBlockState state, Entity entityIn) {
		if (state.getValue(BlockAnomalySteam.ANOMALYSTAGE) == 0) {
	...
	public AxisAlignedBB getCollisionBoundingBox(final IBlockState blockState, final IBlockAccess worldIn,
			final BlockPos pos) {
		return BlockAnomalySteam.NULL_AABB;
	}
	...
	public IBlockState getStateForPlacement(final World worldIn, final BlockPos pos, final EnumFacing facing,
		...
		return this.getDefaultState().withProperty((IProperty) BlockAnomalySteam.ANOMALYSTAGE, 0);
	}
	...
	public AxisAlignedBB getBoundingBox(final IBlockState state, final IBlockAccess source, final BlockPos pos) {
		return BlockAnomalySteam.DEFAULTAABB;
	}
	...
	public void updateTick(final World worldIn, final BlockPos pos, final IBlockState state, final Random rand) {
		if (!worldIn.isRemote) {
			int currentStage = state.getValue(BlockAnomalySteam.ANOMALYSTAGE);
			if (currentStage == 0)
				return;
	...
	private void checkPressed(final IBlockState state, final World worldIn, final BlockPos pos) {
		...
		final boolean flag2 = (boolean) state.getValue((IProperty) BlockAnomalySteam.ANOMALYSTAGE);
		if (flag && !flag2) {
			worldIn.setBlockState(pos, state.withProperty((IProperty) BlockAnomalySteam.ANOMALYSTAGE, (Comparable) true));
			worldIn.markBlockRangeForRenderUpdate(pos, pos);
		}
		if (!flag && flag2) {
			worldIn.setBlockState(pos, state.withProperty((IProperty) BlockAnomalySteam.ANOMALYSTAGE, (Comparable) false));
			worldIn.markBlockRangeForRenderUpdate(pos, pos);
		}
		if (flag) {
	...
	public IBlockState getStateFromMeta(final int meta) {
		...
		return this.getDefaultState().withProperty((IProperty) BlockAnomalySteam.ANOMALYSTAGE, meta);
	}
	...
	protected BlockStateContainer createBlockState() {
		return new BlockStateContainer((Block) this, new IProperty[] { (IProperty) BlockAnomalySteam.ANOMALYSTAGE });
	}
